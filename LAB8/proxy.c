...
int main(int argc, char **argv)
{
    ...
    client再往这个服务器发送数据时，系统会发出一个SIGPIPE信号给进程，告诉进程这个连接已经断开了，
    不要再写了。 
...
　　这时SIGPIPE交给了系统处理。 
*/
    int port = (argc == 2) ? atoi(argv[1]) : DEFPORT;//让端口转化为默认端口
    ...
    pshared参数控制着信号量的类型。如果 pshared的值是0，就表示它是当前里程的局部信号量；否则，
    其它进程就能够共享这个信号量。只对不让进程共享的信号量感兴趣。（这个参数受版本影响），　
    ...
    while (1)
    {
        ...
        connfd = Accept(listenfd, (SA *)(&(argsp->sock)), &clientlen);//返回一个描述符 
        argsp->fd = connfd;
        Pthread_create(&tid, NULL, do_it, (void *)argsp);//创建线程，运行do_it
...
void client_error(int fd, char *cause, char *errnum, 
                 char *shortmsg, char *longmsg)//错误处理
{
...
void *do_it(void *vargs)
{
    ...
    int fd = args->fd;
    struct sockaddr_in sock;
    memcpy(&sock, &(args->sock), sizeof(struct sockaddr_in));//将args—>sock拷贝到sock中
    free(args);//释放
    
    char buf[MAXLINE];
    rio_t rio;
    Rio_readinitb(&rio, fd);//读取描述符
    Rio_readlineb(&rio, buf, MAXLINE);//读取buf
    
    char method[MAXLINE], uri[MAXLINE], version[MAXLINE];//格式
    if (sscanf(buf, "%s %s %s", method, uri, version) < 3)//输入错误处理
    {
    ...
    if (strcmp(method, "GET"))//查看是否是get请求
    {
    ...
    int port;
    if (parse_uri(uri, hostname, pathname, &port)) //错误处理
    {
...
int read_body_and_send(rio_t *rio, char *body, int len, int fd)//读取和发出
{
    int total_size = 0;
    int size;
    while (len > 0)
...
void close_fd(int fd1, int fd2)//关闭描述符，以防溢出
{
    if (fd1 > 0)
        Close(fd1);
